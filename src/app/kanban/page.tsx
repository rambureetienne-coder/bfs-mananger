                    ...
                    <tbody>
                      {deptTasks.map(task => (
                        <tr key={task.id} onClick={() => openEditModal(task)} className="border-b border-slate-800/40 hover:bg-slate-800/50 cursor-pointer transition-colors">
                          ...
                          <td className="px-6 py-4 text-slate-400">{new Date(task.start).toLocaleDateString()}</td>
                          <td className="px-6 py-4 text-slate-400">{new Date(task.end).toLocaleDateString()}</td>
